'''
This codebase builds upon the code provided by MTT: https://github.com/GeorgeCazenavette/mtt-distillation
The procedure to create buffer files is same as that provided in MTT
'''
...
def main(args):
    ...
    syn_lr = torch.tensor(args.syn_rate)
    ...
    if args.pix_init == 'real':
        ...
        for c in range(num_classes):
            ...
            alpha = torch.rand(args.ipc, 1, 1, 1)
            image_syn.data[c * args.ipc:(c + 1) * args.ipc] = alpha*get_images(c, args.ipc).detach().data + (1-alpha)*get_images(c, args.ipc).detach().data
    else:
    ...
    if args.energy == 'cross_entropy':
        criterion = nn.CrossEntropyLoss().to(args.device)
    elif args.energy == 'margin':
        criterion = nn.MultiMarginLoss(p=1,margin=1.0).to(args.device)
    elif args.energy == 'focal':
        m = torch.nn.Softmax(dim=-1)
        criterion = FocalLoss(gamma=0.7).to(args.device)


    # margin = nn.MultiLabelMarginLoss().to(args.device)
    ...
    for it in range(0, args.Iteration+1):
        ...
        x = torch.clone(syn_images).detach()
        # print("size of x : ", x.size())
        this_y = torch.clone(y_hat).detach()
        ...
        for step in range(args.syn_steps):
            ...
            # Gradient Descent on chosen Energy function / Loss function
            if args.energy == 'focal':
                loss = criterion(m(logits), this_y)
            else:
                loss = criterion(logits, this_y) 

            if args.logsumexp:
                loss = loss - torch.logsumexp(logits,1).mean()
                

            grad = torch.autograd.grad(loss, student_params[-1], create_graph=True)[0]

            # Store samples from posterior associated with synthetic set
        ...
        if args.match_trajectory:
            ...
            target_param = target_params.clone().detach()
            student_param = student_params[-1].clone().detach()

        x = syn_images.clone().requires_grad_(True)
        this_y = y_hat.requires_grad_(False)

        
        target_param = target_params.unsqueeze(0).expand(torch.cuda.device_count(), -1).requires_grad_(True)
        student_param = student_params[-1].unsqueeze(0).expand(torch.cuda.device_count(), -1).requires_grad_(True)
        ...
        image_real = image_real.to(args.device)
        label_real = torch.tensor([np.ones(args.ipc,dtype=np.int_)*i for i in range(num_classes)], dtype=torch.long, requires_grad=False, device=args.device).view(-1)
        ...
        neg_logit = student_net(x, flat_param=student_param)
        pos_logit = student_net(x, flat_param=target_param)

        # To calculate contrastive-divergence loss, take the difference between energy/loss from the manifested network
        ...
        if args.logsumexp:
            pos_energy = pos_energy - torch.logsumexp(pos_logit,1).mean()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--syn_rate', type=float, default=1e-2, help='learning rate for synthetic images')
    ...
    parser.add_argument('--logsumexp', default=False, help='use logsumexp in the energy function')
    parser.add_argument('--match_trajectory', default=True, help='match the trajectory of the expert')

    parser.add_argument('--energy', default='cross_entropy', help='energy function to use')
